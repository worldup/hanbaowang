...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			//_initTreeValue('${basicInfo.examType}');
			
		});
		/* function _initTreeValue(examType){
			if(!$.trim(examType)){
				return;
			}
			var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
			var node = treeObj.getNodeByParam("id", examType, null);
			if(node){
				$('#examType').val(node.name);
			}
		} */
	</script>
	...
	<div class="dzwidth_960 user">
		<table class="addform_wrap" id="basicInfo" style="margin:0;">
			<tr>
				<td>
					<label><b>*</b>试卷名称：</label><input name="examName" id="examName" value="${basicInfo.examName}" class="text_170" />
				</td>
				<td >
					<label><b>*</b>试卷类型：</label><input name="examType" id="examType" onclick="showMenu();" readonly="readonly" class="text_170" value="${basicInfo.examTypeName}"/>
				...
				<td>
					<label><b>*</b>答题时间：</label><input name="examTimer" id="examTimer" class="text_170" value="${basicInfo.examTimer}" /><span>&nbsp;&nbsp;分钟</span>
				</td>
				<td rowspan="2">
					<label>说明：</label><textarea name="description" style="width:260px;" id="description">${basicInfo.description}</textarea>
				</td>
			</tr>
			<tr>
				<td>
					<label><b>*</b>试卷总分：</label><input name="totalValue" id="totalValue" readonly="readonly" value="${basicInfo.totalValue}" class="text_170"/>
				</td>
				<td>
					<label><b>&nbsp;</b>通过分数：</label><input name="passValue"  id="passValue" class="text_170" value="${basicInfo.passValue}"/>
				</td>
				<td>
	...
	<div class="txt_center clearfix" style="width:68%">
		<div class="qjact_btn clearfix" >
			<a href="javascript:history.go(-1)"
					class="big_ea"><span>取&nbsp;&nbsp;消</span></a>
			<a href="javascript:void(0)"
					class="big_green" id="updateExam" ><span>保&nbsp;&nbsp;存</span></a>
			<a href="javascript:void(0)"
					class="big_green" id="saveAsAnotherExam" ><span>另存为</span></a>
		</div>
	</div>
	<input id="examId" value="${examId}" type="hidden"/>
	<input id="examTypeId" value="${basicInfo.examType}" type="hidden"/>
	<script type="text/javascript"
	...
	<script type="text/javascript">
		ExaminationPaper.init({
			...
			headValues : $.parseJSON('${headValues}'),
