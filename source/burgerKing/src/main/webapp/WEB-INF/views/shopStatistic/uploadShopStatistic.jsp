	...
	<table class="addform_wrap">
		<tr>
			...
			<td><input type="file" onchange="checkFileSuffix('storeFile')"
				id="storeFile" name="shopStatisticFile" class="files"></td>
		</tr>
		<tr>
			<td class="title"></td>
			<td>
...
<script>
	...
	function ajaxS() {
		...
		var options = {
			url : "${basePath}/shopStatistic/toImport",
			dataType : "text",
			type : 'POST',
			success : function(data, status) {
				data = $.parseJSON(data);
				if (data.success) {
					alert(data.obj || "批量导入成功");
					closeOutWindow();
				} else {
					alert("批量导入失败");
				}
				$("#gridTableShopStatistic").jqGrid().trigger("reloadGrid");
			}
		};
		var form = $('#form1');
